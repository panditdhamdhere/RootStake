...
import List from "../Components/Admin/RegularComp/List";
...
const Notification = ({ index, notify, poolDetails }) => {
  ...
  useEffect(() => {
    const calculatePercentage = () => {
      ...
      if (percentageNew === 0) {
        console.log("Token Blance is zero Cannot calculate percentage!");
      } else {
  ...
  return (
    <div key={index} className="deposit">
      ...
      >
        <h3 className="progressbar__title">Accurate {notify?.amount}</h3>
        <div
          className="progress"
          role="progressbar"
          aria-valuenow={75}
        ...
        <div className="progressbar__values">
          <span className="progress__value progressbar__values--left">0%</span>
          <span className="progress__value progressbar__values--right">
            100%
          </span>
        </div>
      </div>
