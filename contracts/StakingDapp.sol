...
contract Staking is Ownable, ReentrancyGuard {
    ...
    function deposite(uint256 _poolID, uint256 _amount) public nonReentrant {
        ...
        if (user.amount > 0) {
            uint256 pending = calculatePendingReward(user, _poolID);
            pool.rewardToken.transfer(msg.sender, pending);

            createNotification(_poolID, pending, msg.sender, "Claim");
        }
        ...
        // user.lockUntil = block.timestamp + (pool.lockedDays * 86400);
        user.lockUntil = block.timestamp + (pool.lockedDays * 60);

        depositedToken[address(pool.depositToken)] += _amount;

        createNotification(_poolID, _amount, msg.sender, "Deposited");
    }
    ...
    function withdraw(uint256 _poolID, uint256 _amount) public nonReentrant {
        ...
        require(user.amount >= _amount, "Withdraw amount exceeds the balance!");
        ...
        user.lastRewardAt = block.timestamp;
        createNotification(_poolID, _amount, msg.sender, "Withdraw");
    }
    ...
    function calculatePendingReward(
        ...
        if (daysPassed > pool.lockedDays) {
            daysPassed = pool.lockedDays;
        }
    ...
    function pendingReward(
        ...
        return calculatePendingReward(user, _poolID);
    }
    ...
    function sweep(address _token, uint256 _amount) external onlyOwner {
        ...
        require(
            token_balance - _amount >= depositedToken[_token],
    ...
    function modifyPool(uint256 _poolID, uint256 _apy) public onlyOwner {
        PoolInfo storage pool = poolInfo[_poolID];
        pool.apy = _apy;
    }

    function claimReward(uint256 _poolID) public nonReentrant {
        PoolInfo storage pool = poolInfo[_poolID];
        UserInfo storage user = userInfo[_poolID][msg.sender];
        ...
        createNotification(_poolID, pending, msg.sender, "claimed");
    }
